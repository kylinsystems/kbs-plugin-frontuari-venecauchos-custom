 ...
 *  @autor Jorge Colmenarez, 3 oct. 2017, jcolmenarez@frontuari.com, Frontuari, C.A.
 *  Support for BPartner Value, TaxID, Credit 5, 10, 15 and more of 15 Days
 */
public class Aging extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		//	Added By Jorge Colmenarez, 2017-10-03 08:42
			+ " LEFT JOIN LCO_TaxIdType t ON (bp.LCO_TaxIdType_ID = t.LCO_TaxIdType_ID) "  
		//	End Jorge Colmenarez
	...
	/**
	 * Set new Columns for Credit Days
	 * @autor Jorge Colmenarez, 3 oct. 2017, jcolmenarez@frontuari.com, Frontuari, C.A.
	 * @param aging
	 * @param daysDue
	 * @param openAmt
	 */
	public void add(MAging aging, int daysDue, BigDecimal openAmt){
		BigDecimal amt = openAmt;
		//	Not due - negative
		if (daysDue <= 0)
		{
			if (daysDue >= -5 && daysDue <= 0){
				BigDecimal Due5 = (BigDecimal)aging.get_Value("Due5")==null ? Env.ZERO : (BigDecimal)aging.get_Value("Due5");
				aging.set_ValueOfColumn("Due5", Due5.add(amt));
			}

			if (daysDue >= -10 && daysDue <= -6){
				BigDecimal Due10 = (BigDecimal)aging.get_Value("Due10")==null ? Env.ZERO : (BigDecimal)aging.get_Value("Due10");
				aging.set_ValueOfColumn("Due10", Due10.add(amt));
			}
			
			if (daysDue >= -15 && daysDue <= -11){
				BigDecimal Due15 = (BigDecimal)aging.get_Value("Due15")==null ? Env.ZERO : (BigDecimal)aging.get_Value("Due15");
				aging.set_ValueOfColumn("Due15", Due15.add(amt));
			}
				
			if (daysDue <= -16){
				BigDecimal Due16_Plus = (BigDecimal)aging.get_Value("Due16_Plus")==null ? Env.ZERO : (BigDecimal)aging.get_Value("Due16_Plus");
				aging.set_ValueOfColumn("Due16_Plus", Due16_Plus.add(amt));
			}
		}
		else	//	Due = positive (> 1)
		{
			if (daysDue >= 0 && daysDue <= 5){
				BigDecimal PastDue5 = (BigDecimal)aging.get_Value("PastDue5")==null ? Env.ZERO : (BigDecimal)aging.get_Value("PastDue5");
				aging.set_ValueOfColumn("PastDue5", PastDue5.add(amt));
			}
			if (daysDue >= 6 && daysDue <= 10){
				BigDecimal PastDue10 = (BigDecimal)aging.get_Value("PastDue10")==null ? Env.ZERO : (BigDecimal)aging.get_Value("PastDue10");
				aging.set_ValueOfColumn("PastDue10", PastDue10.add(amt));
			}
			if (daysDue >= 11 && daysDue <= 15){
				BigDecimal PastDue15 = (BigDecimal)aging.get_Value("PastDue15")==null ? Env.ZERO : (BigDecimal)aging.get_Value("PastDue15");
				aging.set_ValueOfColumn("PastDue15", PastDue15.add(amt));
			}
			if (daysDue >= 16){
				BigDecimal PastDue16_Plus = (BigDecimal)aging.get_Value("PastDue16_Plus")==null ? Env.ZERO : (BigDecimal)aging.get_Value("PastDue16_Plus");
				aging.set_ValueOfColumn("PastDue16_Plus", PastDue16_Plus.add(amt));
			}
		}
	}
