...
public class ImportInventory extends SvrProcess implements ImportProcess
{
	...
	protected void updateCosting(X_I_Inventory imp, MProduct product,
		...
		if (costingDoc == null) {
			...
			//	Support for add reference to inventory in adjustment document
			costingDoc.set_ValueOfColumn("Ref_Inventory_ID", line.getM_Inventory_ID());
		...
		//	Support for add reference to inventoryline in adjustment document
		costingLine.set_ValueOfColumn("Ref_InventoryLine_ID", line.getM_InventoryLine_ID());
		costingLine.saveEx();
