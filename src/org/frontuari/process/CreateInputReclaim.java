...
public class CreateInputReclaim extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		inv.setDescription("INGRESO POR BX NRO: "+m_rg.getDocumentNo());
		...
		for(MLVEReturnGuaranteeLine line: lines) {
			...
			MInventoryLine il = new MInventoryLine(inv, p_M_Locator_ID, 
					...
					qtyBook,qtyBook.add(line.getQtyEntered()));
		...
		if(inv.processIt(MInventory.ACTION_Complete)) {
			inv.saveEx(get_TrxName());
			m_rg.setM_Inventory_ID(inv.getM_Inventory_ID());
