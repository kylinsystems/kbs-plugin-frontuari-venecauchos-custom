 ...
 *  @author jcolmenarez, 7 nov. 2017
 *  Feature for negate qty and amt when invoice it's N/C
 */
	...
	protected String doIt() throws Exception
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			else 	//	Invoice Basis
			{
				...
				{
					sql.append("SELECT h.C_Currency_ID, ") 
						.append("(CASE WHEN charat(dt.docbasetype, 3) = 'C' THEN -1 ELSE 1 END) * l.LineNetAmt AS LineNetAmt, ") //	Apply Multiplier Factor for LineNetAmt 
						.append("(CASE WHEN charat(dt.docbasetype, 3) = 'C' THEN -1 ELSE 1 END) * l.QtyInvoiced AS QtyInvoiced, ") //	Apply Multiplier Factor for QtyInvoiced 
						.append("NULL, l.C_InvoiceLine_ID, h.DocumentNo,")
						.append(" COALESCE(prd.Value,l.Description),h.DateInvoiced ")
						.append("FROM C_Invoice h")
						.append(" INNER JOIN C_DocType dt ON (h.C_DocType_ID = dt.C_DocType_ID)") //	Support for negate Qty and Amount when Document it's C/N
						.append(" INNER JOIN C_InvoiceLine l ON (h.C_Invoice_ID = l.C_Invoice_ID)")
						.append(" LEFT OUTER JOIN M_Product prd ON (l.M_Product_ID = prd.M_Product_ID) ")
						//	.append("WHERE h.DocStatus IN ('CL','CO','RE')") Remove RE Document
						.append("WHERE h.DocStatus IN ('CL','CO')")
						.append(" AND h.IsSOTrx='Y'")
						.append(" AND h.AD_Client_ID = ?")
				...
				{
					sql.append("SELECT h.C_Currency_ID, ")
						.append("SUM((CASE WHEN charat(dt.docbasetype, 3) = 'C' THEN -1 ELSE 1 END) * l.LineNetAmt) AS Amt, ") //	Apply Multiplier Factor for Amt 
						.append("SUM((CASE WHEN charat(dt.docbasetype, 3) = 'C' THEN -1 ELSE 1 END) * l.QtyInvoiced) AS Qty, ") //	Apply Multiplier Factor for Qty 
						.append("NULL, NULL, NULL, NULL, MAX(h.DateInvoiced) ")
						.append("FROM C_Invoice h")
						.append(" INNER JOIN C_DocType dt ON (h.C_DocType_ID = dt.C_DocType_ID)") //	Support for negate Qty and Amount when Document it's C/N
						.append(" INNER JOIN C_InvoiceLine l ON (h.C_Invoice_ID = l.C_Invoice_ID) ")
