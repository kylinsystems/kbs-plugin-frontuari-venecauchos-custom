...
public class CalloutOrderLine implements IColumnCallout {
	...
	public String start(Properties ctx, int WindowNo, GridTab mTab, 
		...
		if((mTab.getTableName().equals(MInvoiceLine.Table_Name) 
				|| mTab.getTableName().equals(MOrderLine.Table_Name)) 
				&& mField.getColumnName().equals("M_AttributeSetInstance_ID")){
			return calculateprice(ctx,WindowNo,mTab,mField,value,oldValue);
		}
		else
			return null;
	}
	
	public String calculateprice(Properties ctx, int WindowNo, GridTab mTab, 
			GridField mField, Object value, Object oldValue){
		//	Check if Trx is Sales
					...
					if (rs.next())
					{
						...
						if(priceStd.compareTo(priceActual) != 0){
							...
							mTab.setValue("PriceActual", rs.getBigDecimal("PriceStd")
									.subtract(rs.getBigDecimal("PriceList").multiply(discount.divide(new BigDecimal(100)))));
